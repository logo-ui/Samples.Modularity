...
namespace Promisec.ControlCenter.Client.Gui.Modularity
{
    public abstract class PromisecUiModule<TViewModel> : IPromisecUiModule, ICompositionModule<ExtendedSimpleIocContainer>
        where TViewModel : class, IScreen
    {
        ...
        {
            if (container.HasHandler(typeof(IPromisecUiModule), Name) == false)
            {
                container.RegisterHandler(typeof(IPromisecUiModule), null, c => this);
            }
            else
            {
                _rootViewModel = null;
